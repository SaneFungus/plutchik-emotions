...
// Zestawy kolorów etapów: neutralny (Bodziec, Działanie), wyróżniony Impuls, przygaszony Cel
const ACCENT = "text-teal-700 dark:text-teal-400"
const NEUTRAL = "border-slate-200 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800"
const IMPULSE = "bg-slate-50 border-slate-200 hover:bg-slate-100 dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700"
const GOAL = "bg-slate-100 border-slate-200 hover:bg-slate-200 dark:bg-slate-900 dark:border-slate-800 dark:hover:bg-slate-800"

export const EvoChain = ({ emotion, lang }: { emotion: Emotion, lang: Lang }) => {
  ...
  const panel = (step: MechanismStep) => {
    ...
    return (
      <div
        ...
        className="w-full md:basis-full md:order-last text-left rounded-xl border border-l-4 p-4 sm:p-5 animate-fade bg-slate-50 border-slate-200 dark:bg-slate-900 dark:border-slate-700"
      ...
      >
        ...
        {next && (
          <button
            type="button"
            onClick={() => goNext(step)}
            className={`mt-3 -ml-2 px-2 min-h-11 inline-flex items-center gap-1.5 rounded-lg text-sm font-bold cursor-pointer transition-colors focus-visible:outline-2 focus-visible:outline-current ${ACCENT} hover:bg-slate-200 dark:hover:bg-slate-800`}
          >
            {t.next}: {labels[next]}
  ...
  return (
    <div className="flex flex-col md:flex-row md:flex-wrap items-center justify-between gap-3 p-3 sm:p-4 rounded-xl border mb-3 bg-white border-slate-200 not-dark:shadow-sm dark:bg-slate-800/50 dark:border-slate-700">
        {stepButton("stimulus", NEUTRAL, "font-bold text-sm", emotion.stimulus[lang])}
        {open === "stimulus" && panel("stimulus")}
        {arrow("a1")}

        {stepButton("impulse", IMPULSE, "italic text-sm font-serif", <>"{emotion.impulse[lang]}"</>)}
        {open === "impulse" && panel("impulse")}
        {arrow("a2")}

        <div className={`w-full md:w-auto text-center md:flex-1 p-3 rounded-xl border border-current ${emotion.bgLightClass} ${emotion.nameClass}`}>
        ...
        {stepButton("function", GOAL, `font-bold text-xs ${ACCENT}`, emotion.function[lang])}
