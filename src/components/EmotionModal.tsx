...
export const EmotionModal = ({ emotion, lang, isDark, onClose, openerRef, scrollToBodyDictRef }: Props) => {
      ...
      >
        ...
        <div className={`p-4 sm:p-8 flex items-center justify-between gap-4 sticky top-0 z-20 backdrop-blur-md border-b ${isDark ? 'bg-slate-950/80 border-slate-800' : 'bg-slate-50/90 border-slate-200'}`}>
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div>
              <h2 id="emotion-modal-title" className={`text-2xl sm:text-4xl font-black uppercase break-words ${nameColor}`}>{emotion.name[lang]}</h2>
        ...
        <div className="p-6 sm:p-8 space-y-8">
          <section>
            ...
            <EvoChain emotion={emotion} lang={lang} isDark={isDark} />
            <p className="text-xs opacity-70 text-center italic mt-2">{t.modal.neuroception}</p>
          </section>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              <IntensityLadder emotion={emotion} lang={lang} isDark={isDark} />
